...
runname <- "GM13ad_Mundlak_nrep1000_take2" # set a runname
...
source("scripts/updated scripts/mundlak-centering/generative_model_updated_mundlak.R")
...
design <- expand.grid(sample_size = c(200), total_T = c(10, 30), dgm_type = c(1,3,"3a","3d"))
...
for (idesign in 1:nrow(design)) {
    ...
    if (dgm_type %in% c("3d")) {
        coef_NA_fill_mlm <- matrix(NA, ncol = 3, 
                                   nrow = 5, 
                                   dimnames = list(c("(Intercept)", "X", "A", "cluster_mean_X", "cluster_mean_A"), 
                                                   c("Estimate", "Std. Error", "t value")))
        coef_NA_fill_gee <- matrix(NA, ncol = 4, 
                                   nrow = 5, 
                                   dimnames = list(c("(Intercept)", "X", "A", "cluster_mean_X", "cluster_mean_A"), 
    ...
    # for all other models with all fixed effects
        ...
        coef_NA_fill_mlm <- matrix(NA, ncol = 3, 
                                   nrow = 6, 
                                   dimnames = list(c("(Intercept)", "X", "A", "cluster_mean_X", "cluster_mean_A", "X:A"), 
                                                   c("Estimate", "Std. Error", "t value")))
        coef_NA_fill_gee <- matrix(NA, ncol = 4, 
                                   nrow = 6, 
                                   dimnames = list(c("(Intercept)", "X", "A", "cluster_mean_X", "cluster_mean_A", "X:A"), 
    ...
    result <- foreach(isim = 1:nsim, .combine = "c", .errorhandling = "remove", 
        ...
        solution_lmm <- tryCatch(
          {
            if (dgm_type %in% c(1,3)) {
              mlm_fit <- lmer(Y ~ X * A + cluster_mean_X + cluster_mean_A + (1 + A| userid), data = dta)
            } else if (dgm_type == 2) {
              mlm_fit <- lmer(Y ~ X * A + cluster_mean_X + cluster_mean_A + (X * A| userid), data = dta)
            } else if (dgm_type == "3a") {
              mlm_fit <- lmer(Y ~ X * A + cluster_mean_X + cluster_mean_A + (1 | userid), data = dta)
            } else if (dgm_type == "3d") {
              mlm_fit <- lmer(Y ~ X + A + cluster_mean_X + cluster_mean_A + (1 + A | userid), data = dta)
            }
        ...
        solution_gee_ind <- tryCatch(
            {
              if (dgm_type == "3d") {
                gee_ind_fit <- geeglm(Y ~ X + A + cluster_mean_X + cluster_mean_A, id = userid, family = gaussian, 
        ...
        solution_gee_ex <- tryCatch(
          {
            if (dgm_type == "3d") {
              gee_ex_fit <- geeglm(Y ~ X + A + cluster_mean_X + cluster_mean_A, id = userid, family = gaussian, 
        ...
        solution_gee_ar1 <- tryCatch(
          {
            if (dgm_type == "3d") {
              gee_ar1_fit <- geeglm(Y ~ X + A + cluster_mean_X + cluster_mean_A, id = userid, family = gaussian, 
                                   corstr = "ar1", data = dta)
            } else {
              gee_ar1_fit <- geeglm(Y ~ X * A + cluster_mean_X + cluster_mean_A, id = userid, family = gaussian, 
                                   corstr = "ar1", data = dta)
