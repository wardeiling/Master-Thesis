...
dgm_with_treatment <- function(sample_size, total_T, dgm_type) {
    ...
    # compute cluster means of level 1 explanatory variables
    dta$cluster_mean_X <- ave(dta$X, dta$userid, FUN = mean)
    dta$cluster_mean_A <- ave(dta$A, dta$userid, FUN = mean)
