...
# run the simulation
parallel_results <- foreach(isim = 1:nsim, .packages = c("geepack", "lme4", "tidyverse"), .options.RNG=120) %dorng% {
  if (isim %% 10 == 0) {
...
results <- Reduce("+", parallel_results) / nsim

# save results
saveRDS(results, file = paste0("simulation_results_glmm/", runname, "/results.rds"))
