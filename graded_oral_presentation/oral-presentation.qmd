...
1.  Restate the issue in a psychological context <!-- for instance, are we able to understand this bias in the treatment effect in terms of DAGs? -->
2.  Provide practical recommendations <!-- for instance, in what situation is the bias of MLM worrisome and when is it negligible?  -->
...
#| cache: true
#| echo: false
#| fig-cap: "Path Diagram of MLM Without Treatment"
...
|  |  |  |  |
|---------------|---------------------------|---------------|---------------|
| GM | Characteristics | Bias (T = 10) | Bias (T = 30) |
| 1 | Includes random intercept and random slope for treatment | 0.003  | 0.001  |
| 3 | Model 1 with dependency random slope and covariate | -0.051 | -0.023 |

: Simulation results of Parameter Bias present in GM 1 and 3 for N = 200 over 1000 replications, for T = 10 and T = 30

## Treatment Effect Bias: Can DAGs help us?
...
|  |  |  |  |
| 3 |  | -0.051 | -0.023 |
| 3a | Model 3 without random slope $b_{2i}$ | 0.002 | -0.000 |
| 3b | Model 3 without interaction effect $\beta_1$ (between treatment and covariate interaction) | 0.005 | 0.001 |

: Simulation results of Parameter Bias present in GM 3, 3a, 3b for N = 200 over 1000 replications, for T = 10 and T = 30

Conditional independence assumption is still violated for these models...

What is happening here?

<!-- 1. is it only the case that the removal of random slope or interaction results in the dissapearance of bias in this particular generative model? Bad example? -->

<!-- 2. is it the case that this holds more the MLM more generally? Then this may imply that in practice this bias is not a big issue as long as we do not include interaction effects or random slopes in our model. -->
