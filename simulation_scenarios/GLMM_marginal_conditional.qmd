...
waar $\rho = \frac{\sigma_b^2}{\sigma_b^2 + \sigma_\epsilon^2}$ en $\zeta = \frac{\beta_1 \sigma_{X_1}^2}{\beta_1 \sigma_{X_1}^2 + \sigma_b^2 + \sigma_\epsilon^2}$ (see [marginal-relationship-resource](https://bookdown.org/mike/data_analysis/linear-mixed-models.html#random-intercepts-model)). Laten we nu weer data simuleren en zowel de marginale als conditionele relaties tussen de covariaat $X_{it}$ en de uitkomstvariabele $Y_{it}$ onderzoeken, maar we zetten $\sigma^2_{\epsilon} = 0$ zodat dit niet afleidt. Let's now simulate data from this model and directly infer the relationships between the continuous covariate $X_{it}$ and the outcome variable $Y_{it}$.
...
saveRDS(simdata, "simulation_scenarios/output/simdata_section2.2.rds")
```
...
for (i in 1:n_individuals) {
  for (j in 1:n_grid) {
    X1 <- X1_grid[j]
    X2 <- beta_0 + beta_1 * X1 + b0[i] + rnorm(1, mean = 0, sd = sigma_epsilon)
    Y2 <- X2
    Y3 <- beta_0 + beta_1 * X2 + b0[i] + rnorm(1, mean = 0, sd = sigma_epsilon)
...
# Plot the conditional-level curve
plot(X1_grid, Y_conditional, type = "l", lwd = 4, col = "black",
     ylim = range(X1_grid), xlim = c(-5, 5), xlab = expression(X[i1]), ylab = expression(Y[i2]),
...
# Plot the conditional-level curve
plot(Y_conditional, Y_conditional, type = "l", lwd = 4, col = "black",
     ylim = range(X1_grid), xlim = c(-5, 5), xlab = expression(X[i1]), ylab = expression(Y[i3]),
     main = "Effect of X2 on Y3")
# THIS IS DISTORTED BECAUSE WE ARE PLOTTING Y3 AGAINST X1 instead of X2

# Add individual curves (grey lines)
for (i in 1:20) {
  lines(Y_conditional, Y3_individuals[i, ], col = "grey", lwd = 1)
}

# Add the population-level curve (red line)
lines(Y_conditional, marginal_Y3, col = "red", lwd = 4)
...
# or try it so that all other elements are zero.
