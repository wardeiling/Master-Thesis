...
![DAG for Generative Model 1](images/GM1_DAG.png)
...
# Fit models
gee_indep <- geeglm(Y ~ Z + X + Z:X, id = id, data = data, corstr = "independence")
gee_exch <- geeglm(Y ~ Z + X + Z:X, id = id, data = data, corstr = "exchangeable")
gee_ar1 <- geeglm(Y ~ Z + X + Z:X, id = id, data = data, corstr = "ar1")
mlm_mle <- lmer(Y ~ Z + X + Z:X + (1 + X | id), data = data, REML = FALSE)
```
...
treatment_effect <- c(coef(gee_indep)[3], coef(gee_exch)[3], coef(gee_ar1)[3], fixef(mlm_mle)[3])
