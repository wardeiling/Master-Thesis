...
$$
Y_{(t+1)i} = \pi_{0i} + \pi_{1i} Z_{ti} + \pi_{2i} X_{ti} + \pi_{3i} X_{ti} Z_{ti} + e_{(t+1)i} \\ = (\beta_{00} + u_{0i}) + \beta_{10} Z_{ti} +  (\beta_{20} + u_{2i}) X_{ti} + \beta_{30} X_{ti} Z_{ti} + e_{(t+1)i} \\ = \beta_{00} + \beta_{10} Z_{ti} + u_{0i} + X_{ti} (\beta_{20} + \beta_{30} Z_{ti} + u_{2i}) + e_{(t+1)i}
$$
...
The randomization probability $p_t = P(X_{ti} = 1 \mid H_{it})$ is constant at $1/2$. Thus, $X_{ti} \sim \text{Bernoulli}(0.5) \quad \text{for} \quad i = 1, \ldots, N \quad \text{and} \quad t = 1, \ldots, T$. In other words, each individual has an independent $50\%$ chance of receiving the treatment at each time point. The exogenous noise is $e_{(t+1)i} \sim N(0, \sigma_e^2)$.

### Generative Model 2

In GM2, we considered the case with a random intercept and a random slope for $X_{ti}$, a random slope for $Z_{ti}$, and a random slope for the interaction between $X_{ti}$ and $Z_{ti}$; and with a time-varying randomization probability. In addition, the randomization probability is time-varying. The outcome is generated according to the same repeated-observations model presented in GM1. However, the person-level model is different:
