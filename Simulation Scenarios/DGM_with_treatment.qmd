...
ggdag::ggdag_status(GM1_DAG1) + ggdag::theme_dag()
ggdag::ggdag_status(GM1_DAG2) + ggdag::theme_dag()
ggdag::ggdag_status(GM2_DAG1) + ggdag::theme_dag()
ggdag::ggdag_status(GM2_DAG2) + ggdag::theme_dag()
ggdag::ggdag_status(GM3_DAG1) + ggdag::theme_dag()
ggdag::ggdag_status(GM3_DAG2) + ggdag::theme_dag()
```
...
Let's first state generative model 1 according to its original notation by Qian et al. (2020)

$$
\begin{aligned}
Y_{it+1} &= \alpha_0 + \alpha_1 X_{it} + b_{i0} + A_{it} (\beta_0 + \beta_1 X_{it} + b_{i2}) + \epsilon_{it+1} \\
&= \alpha_0 + \alpha_1 X_{it} + b_{i0} + A_{it} \beta_0 + A_{it} X_{it} \beta_1 + A_{it} b_{i2} + \epsilon_{it+1}
\end{aligned}
$$
...
We can see that the order of elements in the final equation aligns exactly with the notation of Qian et al. (2020).
...
The second generative model is given by Qian et al. (2020) as:

$$
\begin{aligned}
Y_{it+1} &= \alpha_0 + \alpha_1 X_{it} + b_{i0} + b_{i1} X_{it} + A_{it} (\beta_0 + \beta_1 X_{it} + b_{i2} + b_{i3} X_{it}) + \epsilon_{it+1} \\ &= \alpha_0 + \alpha_1 X_{it} + b_{i0} + b_{i1} X_{it} + A_{it} \beta_0 + A_{it} X_{it} \beta_1 + A_{it} b_{i2} + A_{it} X_{it} b_{i3} + \epsilon_{it+1}
\end{aligned}
$$

We may now rewrite this into the repeated-observations or within-person model (level 1)

$$
Y_{(t+1)i} = \pi_{0i} + \pi_{1i} Z_{ti} + \pi_{2i} X_{ti} + \pi_{3i} X_{ti} Z_{ti} + e_{(t+1)i}
$$

with the person-level or between-person models (level 2)
...
Now when we directly compare the final equation with the original notation, we can see that the order of elements aligns exactly. 
...
Generative model 3 is the same as GM 1, except that the covariate depends directly on the random intercept. In the original notation this was given by:

$$
X_{i1} \sim N(b_{i0}, 1), \quad X_{it} = Y_{it} + N(b_{i0}, 1) \text{ for } t \geq 2.
$$

whereas in the new notation, this can be written as:
