...
Parameters in marginal models (e.g., GEE) are interpreted as population-level effects
...
-   These are not individual level effects!

-   Example: On the whole, how does aging impact the propensity to develop dementia

Note that the linear structure of the GEE implicitly assumes that $Y_{ij} \perp X_{ik} \mid X_{ij}$, for all $j \neq k$

> The outcome is conditionally independent of explanatory factors at all other times, given the current time's explanatory factors
>
> Translated: The value of the outcome $Y$ at timepoint $t$ is independent of all variates that we are including except the variates measured at timepoint $t$.

This assumption holds with (a) time invariant covariates and (b) deterministic/exogenous time-varying covariates, but not with stochastic/endogenous time-varying covariates.

Let's show how this assumption may be violated:

![](images/clipboard-205847465.png)

As illustrated by the red arrow, $Y1$ is not going to be independent of $X2$ given (that we know) $X1$

problem: it becomes hard to determine how much of the observed effect is the hidden effect of the outcome changing the future covariate value.
...
As explained by Qian et al., 2020, the marginal relationships are given by:
