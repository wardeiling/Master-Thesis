...
Z_i1 <- rnorm(n_i, 0, sigma_Z1)
Y_i2 <- rnorm(n_i, gamma_00 + beta_1 * Z_i1 + u_0i, sigma_e)
Z_i2 <- Y_i2
Y_i3 <- rnorm(n_i, gamma_00 + beta_1 * Z_i2 + u_0i, sigma_e)
...
# 
# for (i in 1:n_i){
#   section2.2data_long$Z[section2.2data_long$id == i & section2.2data_long$time == 1] <- Z_i1[i]
#   section2.2data_long$Y[section2.2data_long$id == i & section2.2data_long$time == 2] <- Y_i2[i]
#   section2.2data_long$Z[section2.2data_long$id == i & section2.2data_long$time == 2] <- Z_i2[i]
...
for (i in 1:n_i){
  section2.2data_long_lagged$Z_lag1[section2.2data_long_lagged$id == i & section2.2data_long_lagged$time == 1] <- Z_i1[i]
  section2.2data_long_lagged$Z_lag1[section2.2data_long_lagged$id == i & section2.2data_long_lagged$time == 2] <- Z_i2[i]
...
section2.2_mlm_reml <- lmer(Y ~ 1 + Z_lag1 + (1 | id), data = section2.2data_long_lagged)
section2.2_mlm_mle <- lmer(Y ~ 1 + Z_lag1 + (1 | id), data = section2.2data_long_lagged, REML = FALSE)
# Generalized Estimating Equations
section2.2_gee_ind <- gee(Y ~ 1 + Z_lag1, id = id, data = section2.2data_long_lagged, family = gaussian, corstr = "independence")
section2.2_gee_exch <- gee(Y ~ 1 + Z_lag1, id = id, data = section2.2data_long_lagged, family = gaussian, corstr = "exchangeable")
section2.2_gee_ar1 <- gee(Y ~ 1 + Z_lag1, id = id, data = section2.2data_long_lagged, family = gaussian, corstr = "AR-M", Mv = 1)
section2.2_gee_unstr <- gee(Y ~ 1 + Z_lag1, id = id, data = section2.2data_long_lagged, family = gaussian, corstr = "unstructured")
# Generalized linear model (OLS): assumes independence of observations
section2.2_glm <- glm(Y ~ 1 + Z_lag1, data = section2.2data_long_lagged, family = gaussian) 
# Marginal linear model (GLS)
section2.2_gls_symm_mle <- nlme::gls(Y ~ 1 + Z_lag1, data = section2.2data_long_lagged, correlation = corSymm(form = ~ 1 | id), method = "ML") 
section2.2_gls_compsymm_mle <- nlme::gls(Y ~ 1 + Z_lag1, data = section2.2data_long_lagged, correlation = corCompSymm(form = ~ 1 | id), method = "ML")
...
marginal_effect_Z1_Y2 <- gamma_00 + beta_1*Z1_mean
marginal_effect_Z2_Y3 <- (1 - rho * zeta - rho) * gamma_00 + ((1 - rho * zeta) * beta_1 + rho)*Z2_mean
```
