...
In dit document zal ik de data-genererende modellen gepresenteerd door Qian et al. (2020) opnieuw creëren, vergezeld door visuele weergaven van deze modellen.

Laten we eerst enkele hoofdpunten van GEE samenvatten.
...
![](images/clipboard-3131633457.png)
...
> Door de uitkomst te laggen, hebben we in wezen drie tijdspunten: $X_{i1}$, $X_{i2} = Y_{i2}$, and $Y_{i3}$.
...
Note that there is an open biasing path from $Y_2$ to $Z_2$ in the DAG: the predictor/covariate $Z_2$ is *caused by* (in this case equivalent to) the previous outcome $Y_2$---and thus this time-varying covariate is *endogenous*. In addition, $u_0$ is a confounder in the relationship between $Y_2$ and $Y_3$ and thus also in the relationship between $Z_2$ and $Y_3$ since $Y_2 = Z_2$. Therefore the "full covariate conditional mean (FCCM)" assumption does not hold:

$$
E[Y_{it+1} \mid Z_{it}] = E[Y_{it+1} \mid X_{i1}, \ldots, X_{iT}]
$$

> Deze formule (equation 5 of Qian) zegt dat de uitkomst op $t+1$ conditioneel onafhankelijk is van uitleggende factoren/covariaten op alle andere tijdspunten (incl. toekomst en verleden) gegeven dat we de waarden weten van de uitleggen de factoren covariaten op het huidige tijdstip $t$.
...
In @fig-pathdiagram.section2.2 it becomes clear that the marginal relationship between $Z_1$ and $Y_2$ is quite simply

$$
E[Y_{i2} \mid Z_{i1}] = \gamma_{00} + \beta_1 Z_{i1}
$$

the relationship between $Z_2$ and $Y_3$ is more complex

$$
E[Y_{i3} \mid Z_{i2}] = (1- \rho \zeta - \rho ) \gamma_{00} + [(1-\rho \zeta)  \beta_1 + \rho] Z_{i2}
$$

Specifieker gezegd, de interpretatie van de slope $\beta_1$ als population level effects is alleen valide voor het effect van $Z_1$ op $Y_2$. Daarom kunnen we wel zeggen dat

> $\beta_1$ is the impact of variate $Z_1$ on average on $Y_2$, across the whole population (assuming other variates fixed)
