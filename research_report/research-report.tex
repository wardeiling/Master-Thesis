...
effect was assessed with analytical multilevel models.
...
which are also a special case of this general model. We refer to the
general model as GM G and the special cases of this model as GM A, B and
C. Table~\ref{tbl-gm-differences} summarizes the differences between the
...
this model into the repeated-observations or within-person model:
...
other. The covariate is generated as
...
GM A is the same as GM G, except that the covariate \(X_{it}\) is not
directly determined by the random intercept \(b_{i0}\) (see
Figure~\ref{fig-GMA_path}):
...
GM B is the same as GM G, except that the random slope \(b_{i2}\) for
the treatment \(A_{it}\) is removed (see Figure~\ref{fig-GMB_path}). The
...
GM C is the same as GM G, except that the interaction term
