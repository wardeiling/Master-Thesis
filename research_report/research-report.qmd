...
We consider two GMs from @qian2020, one (GM A) being a special case of the general model (GM G) where bias was detected. To further isolate the source of bias, we introduce two additional special cases, labeled GM B and C. We first describe the details of the general generative model (GM G) and then move on to its three special cases GM A, B and C.
...
The performance of the models was assessed based on bias through 

The performance of the models was assessed based on the bias and mean squared error (MSE) of the treatment effect estimates. The bias was calculated as the difference between the estimated treatment effect and the true treatment effect, while the MSE was calculated as the squared difference between the estimated treatment effect and the true treatment effect. The bias and MSE were averaged across the 10,000 replications for each setting.
