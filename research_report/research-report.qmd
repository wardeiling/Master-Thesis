...
In this section, we formulate several generative models (GMs) and define specifications for the simulation study.
...
All data generation and estimation was performed in `R`, version 4.4.2 [@rcoreteam2024]. Choices in the simulation-setup were guided by the need for consistency with the methodology of @qian2020. After the generation of data for each setting, analytical MLMs were fit to the data. Specifically, the MLM from @eq-modelG was fit for GM G and A, the MLM from @eq-modelB for GM B, and the MLM from @eq-modelC for GM C[^2]. To fit the MLM, the `lmer` function from the R-package `lme4` [@bates2015] was employed with restricted maximum likelihood estimation.

[^2]: Contrary to the data generating models, the analytical models do not simultaneously model the covariate $X_{it}$ and the treatment $A_{it}$.

In the simulation study, we evaluated the performance of the analytical models across different settings by systematically varying the following 3 factors: Generative Model (GM) with the levels G, A, B, and C; Number of timepoints (T) with the levels 10 and 30; and Sample size (N) with the levels 30, 100, and 200. By varying these factors, we created a total of 24 unique settings, each of which was replicated 10,000 times.
...
In this section, we first evaluate the utility of the conditional independence assumption and the backdoor criterion in detecting bias in treatment effect estimates. We then connect predictions based on these criteria to the simulation study results, which present the performance of the analytical models across different settings.
...
We now apply the backdoor criterion to these DAGs to assess potential violations. The application of the backdoor criterion begins by identifying all paths that connect the treatment $A_t$ to the outcome $Y_{t+1}$, excluding the direct treatment effect itself. Note that if the treatment has a parent, an indirect pathway may exist through the treatment, linking it to the outcome. Next, we determine whether these paths can transmit association by applying the d-separation rules[^3]. A path that contains an arrow pointing to the treatment $A_t$ is considered a *backdoor path*.

[^3]: See [@hayduk2003] for an acccesible introduction to d-separation.
...
Regarding the backdoor criterion and DAGs [@pearl1988; @pearl2009], our results suggest that the classical non-parametric DAG may be insufficient to identify bias in GM G. While DAGs account for direct causal effects, they do not impose restrictions on random slopes or interaction effects[^4], which are central to the conditional independence assumption. Similar concerns regarding the use of the DAG with Pearl's backdoor criterion in situations with interaction effects have been raised [@weinberg2007; @attia2022]. Future research could explore to what extent proposed extensions of the DAG---that incorperate interaction effects---may allow the backdoor criterion to identify bias in the treatment effect estimates.

[^4]: Note that the term "effect modification", while often used interchangeably with "interaction", has a distinct definition in the counterfactual framework [@vanderweele2009].
...
Another avenue for future investigation is the role of centering approaches (see @hamaker2020 for an overview). Namely, @antonakis2021 notes that the assumption of uncorrelatedness between the random effects and level 1 covariates can be relaxed by using Mundlak's contextual model[^5] [@mundlak1978]: adding the cluster means of each covariate as predictor of the random intercept. Such an approach of explicitly modeling the source of endogeneity, as advocated by @bell2015, may further clarify the treatment effect bias in GM G.

[^5]: This is referred to as the Correlated Random Effects (CRE) approach by @wooldridge2002.
