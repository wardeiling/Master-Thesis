...
## Scenario 1: SCM with random intercept and a time-varying $X$ *without* between-person differences (a mean of 0 for everyone)

The generative model for the data is as follows:

$$Y_{it} = \alpha_0 + \alpha_1 X_t + b_{0i} + b_{1} X_{it}+ \epsilon_{it} \quad \text{where} \quad \epsilon_{it} \sim \mathcal{N}(0, 1), \text{iid}$$
where 

- $\alpha_0$: The overall intercept, representing the baseline level of $Y_{t}$ when all other terms are zero.
- $\alpha_1$: The fixed slope of the predictor $X_t$ on $Y_{t}$
- $b_{0i}$: Random intercept for individual $i$, representing deviations from the overall intercept  $\alpha_0$.

Given $X_{it}$ has no between-person differences and a mean of 0:

$$X_{it} = \mathcal{N}(0, 1)$$
Let us give the parameters arbitrary values

- $\alpha_0 = 2$
- $\alpha_1 = 0.6$
- $b_{0i} \sim \mathcal{N}(0, 1)$

Let's specify the structural causal model (SCM) including the effects:

```{r}
# Parameters
set.seed(123)  # For reproducibility
n <- 100  # Number of individuals
total_T <- 5  # Number of measurements per individual

alpha_0 <- 2  # Overall intercept
alpha_1 <- 0.6  # Fixed effect of X on Y
b_0i <- rnorm(n, mean = 0, sd = 1)  # Random intercepts
...
## Scenario 2: SCM with random intercept and a time-varying $X$ *with* between-person differences (one participant has a greater mean than another across the timepoints)

## Scenario 3: SCM with random intercept and a time-varying $X$ *with* between-person differences (one participant has a greater mean than another across the timepoints) that are related to the intercept (of $Y$)
