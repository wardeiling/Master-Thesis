...
as.numeric(VarCorr(scenario1.MLM.1a_mle)$id[1]) # aligns with specification of sigma_u0
```
...
Note that by using the within-person centered predictor $X_{it} - \bar{X}_{i}$, we disentangle the within-person and between-person slopes, whereas the use of the non-centered predictor $X_{it}$ would have yielded a weighted sum of the estimated slopes at both levels.
...
# Simulate data
X_bar_i = rnorm(n_i, mu_X_bar, sigma_X_bar)
u_0i = rnorm(n_i, 0, sigma_u0)
...
for (i in 1:n_i) {
  for (t in 1:n_t) {
    ...
    X_it <- X_bar_i[i] + rnorm(1, mean = 0, sd = sigma_X)
    
    e_it <- rnorm(1, mean = 0, sd = sigma_e)
    
    Y_it <- gamma_00 + u_0i[i] + beta_1 * (X_it - X_bar_i[i]) + e_it
    
    # Store the values in the data frame
    scenario2.data <- rbind(scenario2.data, data.frame(id = i, time = t, X = X_it, Y = Y_it, X_bar = X_bar_i[i]))
  }
}
...
first, we have to prepare the centered within clusters variable $X_{it} - \bar{X}_{i}$
...
```{r}
scenario2.MLM.1a_reml <- lmer(Y ~ 1 + X_cwc + (1 | id), data = scenario2.data, REML = TRUE)
scenario2.MLM.1a_mle <- lmer(Y ~ 1 + X_cwc + (1 | id), data = scenario2.data, REML = FALSE)
...
scenario2.coefs <- data.frame(
  row.names = c("Intercept", "X"),
  coef_MLM_reml = fixef(scenario2.MLM.1a_reml),
  coef_MLM_mle = fixef(scenario2.MLM.1a_mle),
...
The point estimates are essentially identical with $n = 10.000$ across all conditions. Note that for the unstructured working correlation matrix, the warning "Working correlation estimate not positive definite" was given.
...
## Scenario 3: SCM met een random intercept, time-varying $X$ en *met* between-person verschillen in $\bar{X}_{i}$ (de ene proefpersoon heeft een hogere gemiddelde dan een ander over de tijdspunten) die gerelateerd zijn aan de intercept (van $Y$)
...
$$\beta_{0i} = \gamma_{00} + \gamma_{01} \bar{X}_i + u_{0i} \quad \text{where} \quad u_{0i} \sim \mathcal{N}(0, \sigma^2_{u0})$$

in which we predict the within-person mean on the outcome from the within-person mean on the predictor. In other words, by including the between-person slope $\gamma_{01}$ multiplied by the person mean $\bar{X}_i$, we could investigate contextual effects.

By plugging the between person equation into the within-person equation we get the combined expression

$$Y_{it} = \gamma_{00} +  \gamma_{01} \bar{X}_i + u_{0i} + \beta_1 (X_{it} - \bar{X}_{i}) + e_{it}$$
...
for (i in 1:n_i) {
  for (t in 1:n_t) {
    ...
    Y_it <- gamma_00 + gamma_01 * X_bar_i[i] + u_0i[i] + beta_1 * (X_it - X_bar_i[i]) + e_it
    
    # Store the values in the data frame
    scenario3.data <- rbind(scenario3.data, data.frame(id = i, time = t, X = X_it, Y = Y_it, X_bar = X_bar_i[i]))
  }
}
...
```{r}
scenario3.data$X_cwc <- scenario3.data$X - scenario3.data$X_bar
```

Now let's fit the models

```{r}
scenario3.MLM.1a_reml <- lmer(Y ~ 1 + X_cwc + X_bar + (1 | id), data = scenario3.data, REML = TRUE)
scenario3.MLM.1a_mle <- lmer(Y ~ 1 + X_cwc + X_bar + (1 | id), data = scenario3.data, REML = FALSE)

summary(scenario3.MLM.1a_reml)

as.numeric(VarCorr(scenario3.MLM.1a_mle)$id[1]) # aligns with specification of sigma_u0
```
...
scenario3.coefs <- data.frame(
  ...
  coef_MLM_reml = fixef(scenario3.MLM.1a_reml),
  coef_MLM_mle = fixef(scenario3.MLM.1a_mle),
