# Created by: 2018.10.22 Tianchen Qian
# Edited by: 2024.11.17 Ward B. Eiling
...
source("Scripts/Simulation/GMs_translated.R")
...
set.seed(120)
nsim <- 20 # change when running full simulation
...
for (idesign in 1:nrow(design)) {
    ...
    set.seed(123)
    result <- foreach(isim = 1:nsim, .combine = "c") %dopar% {
        if (isim %% 10 == 0) {
...
test <- readRDS("simulation_result/1.RDS")
...
for (idesign in 1:nrow(design)) {
    ...
    alpha_1 <- sapply(result, function(l) l$coef["Z", "Estimate"])
    alpha_1_sd <- sapply(result, function(l) l$coef["Z", "Std. Error"])
    alpha_1_df <- sapply(result, function(l) l$coef["Z", "df"])
    ...
    beta_1 <- sapply(result, function(l) l$coef["Z:A", "Estimate"])
    beta_1_sd <- sapply(result, function(l) l$coef["Z:A", "Std. Error"])
    beta_1_df <- sapply(result, function(l) l$coef["Z:A", "df"])
