...
# coef(summary(lmer(Y ~ Z * X + (1 + X | userid), data = dta)))["X", "Estimate"]
# coef(summary(geeglm(Y ~ Z * X, id = userid, data = dta, corstr = "independence")))["X","Estimate"]
# fixef(lmer(Y ~ Z * X + (1 + X | userid), data = dta))
